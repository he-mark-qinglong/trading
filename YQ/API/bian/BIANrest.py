# -*- coding: utf-8 -*-
"""
Created on Thu Jun  3 10:11:36 2021
...
class BIANrest(binance):
    def __init__(self, apiKey='', secret=''):
        super(BIANrest,self).__init__()
        binance.apiKey = apiKey
        binance.secret = secret
        ex = ccxt.binance()
#        print(ex.load_time_difference())
#        ex.options.update({'defaultType': 'delivery'})
        ex.options.update({'timeDifference': -60000})
        ex.options.update({'adjustForTimeDifference':True})
        ex.options.update({'broker': {'spot': 'x-FPVJ8J8S','margin': 'x-FPVJ8J8S','future': 'x-XZlmNuiD','delivery': 'x-XZlmNuiD'}})
        
    def import_config(self):
       
        filepath = 'YQ/API/bian/market_config.json'
#        filepath = 'market_config.json'
...
#        print(info)
    ...
    def get_spot_depth(binance,symbol,limit=50):
        '''
        :param symbol: BTCUSDT, BTCUSD_PERP, BTCUSD_210625
        :param limit: 默认500 最大1500
        :return:
...
#        print(info)
    ...
    def get_spot_bid_ask(binance, symbol):
        '''
        :param symbol: BTCUSDT, BTCUSD_PERP, BTCUSD_210625
        :param limit: 默认500 最大1500
        :return:
...
#        print(info)
        ...
        return binance.publicGetTickerBookTicker(params) 
    ...
    def create_spot_order(binance, symbol, side, price, amount ,type='LIMIT'):
        '''
       ...
       #oco订单one fill one cancel ,即同时挂2个订单出去,一个订单成交,另一个就取消,一般用于同时下止盈止损单,要么触发止盈,要么触发止损 
        ...
        return binance.privatePostOrder(params) 
    def get_spot_ExchangeInfo(binance):
        
         return binance.publicGetExchangeInfo()
...
#        return binance.fapiPublicGetExchangeInfo(params) 
    ...
    def cancel_all_spot_order(binance, symbol):
        symbol = symbol.replace('/', '').upper()
        params = {'symbol': symbol,
                  'timestamp': int(time.time() * 1000)
                  }
        print(111111111111111111111111111111111)
        return binance.privateDeleteOpenOrders(params) 
...
#        } if base_type == 'coin' else {}
    ...
    def get_spot_balances(self):
        timestam=self.get_serverTime()
        timestam=timestam['serverTime']
        params = {
            'timestamp': timestam
        }
        return super(BIANrest,self).privateGetAccount(params)
#        return binance.privateGetAccount(params)
    ...
    def get_serverTime(self):
        url = '/api/v3/time'
#        url = '/api/v1/private?command=returnCompleteBalances'
        
        data = self.send_req('GET',url,{})
        
        return data
    def send_req(self,act ,url,params = ''):
        # if not params:
        url = resturl + url
        data = requests.request(act,url,params = params)
        try:
